...
public class EventAggregator implements Comparator<XbrEvent>
{
	...
	{	
		...
		// holds statistics for the same sequence # read by different reader locations
		...
		// now, walk through the events, breaking every time there's a new bandid or packet # or reader location
		for (XbrEvent ev: alEvents)
		{
			...
			if (ri==null)
			{
				logger.error("Received event for unknown reader: " + ev.getReaderName());
				continue;
			}
			...
			if (liNew == null) 
			{
				logger.warn("Ignoring event from reader " + ri.getName() + " because it is not assigned to a valid location");
				continue;
			}
			...
			{
				TapEventAggregate tea = new TapEventAggregate(ev.getID(), ((TapEvent) ev).getPidDecimal(), ev.getReaderName(), ev.getTime(), (TapEvent)ev);
				liAggregated.add(tea);
				continue;
			}
			
			if (ev instanceof AVMSEvent)
			{
				AVMSEventAggregate ea = new AVMSEventAggregate(ev.getID(), ev.getReaderName(), ev.getTime(), (AVMSEvent)ev);
				liAggregated.add(ea);
			}
			
			if (ev instanceof XtpGpioEvent)
			{
				XtpGpioEventAggregate ea = new XtpGpioEventAggregate(ev.getID(), ev.getReaderName(), ev.getTime(), (XtpGpioEvent)ev);
				liAggregated.add(ea);
			}
			
			// ignore all but LrrEvents
			...
			{
				logger.trace("**** Comparing band ids:  " + lsCur.getID() + " vs " + ev.getID());
				logger.trace("**** Comparing pnos:      " + lsCur.getPacketSequence() + " vs " + le.getPno());
				logger.trace("**** Comparing locations: " + lsCur.getLocationInfo().getName() + " vs " + sLocationNew);
			}
			...
			if ( (lsCur==null) ||
			     ...
			     (lsCur.getLocationInfo().getName().compareTo(sLocationNew)!=0) ) 
		    {
				// if it's the 1st event, just set up for that
				if (lsCur==null)
				{
					lsCur = new LocationStats(le.getID(), le.getPno(), controller.getReader(le.getReaderName()), le.getTime(), le.getXband(), le.getGuest());
					lsCur.setCount(1);
						  ...
						  (lsCur.getPacketSequence() != le.getPno()) )
				{
					...
					// band id has changed or packet sequence has changed - emit best location
				...
				{
					// the only case that's left is that the location info has changed for the same band id and packet sequence #
					// In this case, we start collecting data for the new location
					lsCur = new LocationStats(le.getID(), le.getPno(), controller.getReader(le.getReaderName()), le.getTime(), le.getXband(), le.getGuest());
					lsCur.setCount(1);
					lsCur.setMaxSS(le.getSs());
					lsCur.setSumSS(le.getSs());
		...
		// now, sort the aggregate list. Note that we're conveniently using the last instance
		// of lsCur as our comparator. If lsCur is null, that means that there's nothing that
	...
	private void
	AddAggregateForBestLocation(List<EventAggregate> liAggregated, List<LocationStats> liLocStats)
	{
		// singulate the aggregate events
		EventAggregate ea = sing.Singulate(liLocStats, controller.getSingulationAlgorithm());
		liAggregated.add(ea); 
	}
