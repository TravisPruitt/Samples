...
public class SchedulerItemAction extends BaseAction implements Preparable {
	...
	public String saveItem() throws Exception {
		...
		try {
			client.saveItem(item, getPrincipal() != null ? getPrincipal().getName() : null, serverUrl);
			return "saved";
		}
		catch(Exception e) {
	...
	public String deleteItem() throws Exception {
		...
		try {
			client.deleteItem(itemKey, getPrincipal() != null ? getPrincipal().getName() : null, serverUrl);
			return "saved";
		}
		catch(Exception e) {
	...
	private void initialize() throws Exception {
		...
		if (isNewItem()) {
			...
			items = client.getItems(serverUrl);
			...
			Collection<SchedulerItemMetadata> mdList = client.getMetadata(serverUrl);
			...
			item = client.getItem(itemKey, serverUrl);
		}
