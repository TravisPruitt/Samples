...
ProcessAttractionElement(xmlNode *pnode)
{
    ...
    pAttraction->priEntry = pAttraction->priExit = pAttraction->priMergeDap = pAttraction->priXPassDap = NULL;
...
ProcessReadersElement(xmlNode *pnode)
{
	...
	for (pc=pnode->children; pc!=NULL; pc=pc->next)
	{
        if (pc->type == XML_ELEMENT_NODE)
        {
            ...
            if (strncmp(pszName, "entry", 5)==0)
            	pAttraction->priEntry = &pReaders[iChild];
            else if(strncmp(pszName, "exit", 4)==0)
            	pAttraction->priExit = &pReaders[iChild];
            else if(strcmp(pszName, "xpassentry")==0)
            	pAttraction->priXPassDap = &pReaders[iChild];
            else if(strcmp(pszName, "merge")==0)
            	pAttraction->priMergeDap = &pReaders[iChild];
...
ProcessGuestsElement(xmlNode *pnode)
{
	...
	// fetch the xpass probability
	pGuests->nXPassProbability = GetIntAttribute(pnode, "xpassprobability", 0);
