...
public class CLOptions
{
	...
	public CLOptions()
	{
		...
		// set start time to NOW, pretending we're in park time
		nStartHour = calBase.get(Calendar.HOUR_OF_DAY);
		nStartMin = calBase.get(Calendar.MINUTE);
	...
	{
		System.out.println("JmsTalker - generates JMS messages based on xml");
		System.out.println("Usage: java -jar JmsTalker.jar [OPTIONS] <file.xml>");
		...
		System.out.println("   --from HOUR[:MIN]    Starts simulation at the given time (default NOW)");
		System.out.println("   --to HOUR[:MIN]      Ends the simulation at the given time (default 23:59)");
