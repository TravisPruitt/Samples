        ...
        columns.forEach(function(column) {
            if (column.value === null) {
                ...
                var url = function(row) {
					...
					return ['http://', facility(value(row, 'FacilityID')) , '/Xfpe/restful/', value(row, 'ReaderPrefix'), '-', value(row, 'EventTypeName').toLowerCase(), '-', value(row, 'ReaderPosition').toLowerCase(), '/rfid/tap?uid=', value(row, 'xbandId')].join('');//&pid=12312312&sid=12312312312
                };
                try {
