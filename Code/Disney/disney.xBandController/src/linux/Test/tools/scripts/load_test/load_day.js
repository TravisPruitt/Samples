        ...
        columns.forEach(function(column) {
            if (column.value === null) {
                ...
                var url = function(row) {
					...
					var readerName=([value(row, 'ReaderName').toLowerCase(), '-', value(row, 'EventTypeName').toLowerCase(), '-', value(row, 'ReaderPosition').toLowerCase()].join(''));

					if(isHardwareReader(readerName)) {
						//Hardware xTP restful call example:
						//http://10.110.1.110:8080/rfid/tap?uid=54E03ABD32103323&pid=123456&sid=890ABC
						return ['http://', facility(value(row, 'FacilityID')) , '/rfid/tap?uid=', value(row, 'tapid'), '&pid=123456', '&sid=890ABC'].join('');					
					}
					else {
						//Software xTP restful call example:
						//http://10.110.1.63:8090/Xfpe/restful/phil-entry-l/rfid/tap?uid=F0B2E8E487A800
						return ['http://', facility(value(row, 'FacilityID')) , '/Xfpe/restful/', value(row, 'ReaderName').toLowerCase(), '-', value(row, 'EventTypeName').toLowerCase(), '-', value(row, 'ReaderPosition').toLowerCase(), '/rfid/tap?uid=', value(row, 'tapid')].join('');
					}
                };
                try {
