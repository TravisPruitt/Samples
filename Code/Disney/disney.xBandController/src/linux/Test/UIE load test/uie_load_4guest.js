...
//******* MK LDU ********
//var uie_host = 'nge-mk-mobile.wdw.disney.com' //10.93.0.7
//var uie_host = '10.93.0.7';
//***********************
...
//This test assumes that the IDMS database contains a set of unassociated bands with sequential
// secureID values.  set firstSecureId to the value of the first band in the set. bandCount is the
// number of bands in the set.
firstSecureId = 40000023;
bandCount = 5000;
...
var globalSecureId = firstSecureId;
var maxSecureId = ((parseInt(process.argv[3], 10) + parseInt(firstSecureId, 10)) * 1 || firstSecureId + bandCount) * 1;
...
var doBooking = function() {
	...
	request({
		...
		request({
			...
			if(utils.checkResponse(response2, thread_id) != 0) {
				error_count++;
			}
			...
			request({
				...
				if(utils.checkResponse(response3, thread_id) != 0) {
					error_count++;
				}
				...
				request({
					...
					if(utils.checkResponse(response4, thread_id) != 0) {
						error_count++;
					}
					...
					request({
						...
						if(utils.checkResponse(response5, thread_id) != 0) {
							error_count++;
						}
						...
						//var offerset = Math.floor(Math.random()*2); 
