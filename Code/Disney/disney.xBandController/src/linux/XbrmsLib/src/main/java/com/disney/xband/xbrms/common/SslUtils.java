...
import com.disney.xband.ac.lib.client.*;

public abstract class SslUtils {
	
    private static int CONNECTION_TO = 3000;
	...
	static
	{
		 ...
		 * ssl configuration
		 */
		String sslClientKeystore = ConfigProperties.getInstance().getProperty("ssl.keyStore.client");
		String sslClientKeystorePassword = ConfigProperties.getInstance().getProperty("ssl.keyStorePassword.client");
...
//		}
	...
	public static HttpURLConnection getHttpConnection(URL url) throws IOException
	{
		...
		conn.setConnectTimeout(CONNECTION_TO);
               ...
               try {
                   ...
                   if(fis != null) {
                       try {
                    	   ...
                    	   logger.warn("File input stream failed to close. Possible memory leak.", e);
                       }
                   }
               }
