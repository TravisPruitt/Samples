 ...
 */
public class XconnectRollingFileAppender extends FileAppender {
	...
	/**
	 * Allow optional compression of backed up files.
	 */
	protected boolean gzip = false;
	
	/**
	 * Start compressing from the first gzipped file.
	 */
	protected int startGzipIndex = 1;
	 ...
	 */
	public XconnectRollingFileAppender() {
		super();
	}
	 ...
	 */
	public XconnectRollingFileAppender(Layout layout, String filename,
			...
			for (int i = maxBackupIndex - 1; i >= 1 && renameSucceeded; i--) {
				...
				if (file.exists()) {
					...
					if (file.exists()) {
						...
						if (gzip && startGzipIndex == i+1)
							compressFile(target.getAbsolutePath());
					}
				}
			}
	...
	public int getStartGzipIndex() {
		return startGzipIndex;
	}

	public void setStartGzipIndex(int startGzipIndex) {
		this.startGzipIndex = startGzipIndex;
	}
	...
	class CompressThread implements Runnable
	{
		...
		public void run()
		{
			...
			{
				LogLog.error("Failed to gzip file " + filePath, e);
			}
		}
	}
