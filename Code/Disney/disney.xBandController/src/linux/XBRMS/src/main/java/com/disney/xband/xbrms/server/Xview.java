...
public class Xview {
	...
	private Timer timer;
	private final int GUEST_CACHE_CLEANUP_SEC = 60;
	private final String XVIEW_URL = "http://localhost:8090/Xview";
	...
	private void doCleanup() {
		Long expireTime = new Date().getTime() - GUEST_CACHE_CLEANUP_SEC * 1000;
	...
	public Guest getGuestByGuestId(String guestId) {
		...
		ge = new GuestEntry();
		Guest guest = getGuestFromXview(XVIEW_URL + "/guests/id/" + guestId);
		if (guest == null)
			return null;
	...
	public Guest getGuestByName(String firstName, String lastName) {
		...
		ge = new GuestEntry();
		Guest guest = getGuestByNameFromXview(XVIEW_URL + "/guests/search/" + firstName + "%20" + lastName);
		if (guest == null)
			return null;
	...
	private Guest getGuestByNameFromXview(String urlString)
	{
		...
		try
		{
			...
			HttpURLConnection conn = (HttpURLConnection) SslUtils.getConnection(url);
			if (conn.getResponseCode()>=400)
			{
				logger.error("Error: " + conn.getResponseCode() + " received from xView (" + url.toString() + ")");
				return null;
