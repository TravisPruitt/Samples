...
RFIDTag::RFIDTag(ByteArray& uid) : _uid(uid)
{
}
...
bool RFIDTag::parsePublicData(ByteArray& bytes)
{
    ...
    // NOTE: Some early cards had an authKeyVersion of 0, but should have been 1.
    //       It is believed this was limited primarly to a few development cards
    //       and the first 10,000 cards provisioned by John D. at Disney.
    //       The mid-term solution was to treat auth key version 0 as a 1 and not
    //       use key version 0 on any cards. Long term the following statement
    //       could be yanked once we are certain the early cards are out of
    //       circulation.
    if (_authKeyVersion == 0)   _authKeyVersion = 1;
