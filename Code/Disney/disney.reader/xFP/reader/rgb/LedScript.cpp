...
LedScript::LedScript(RGBCluster* cluster) :
    ...
    locked(false), xBioBrightness(DEFAULT_XBIO_BRIGHTNESS),
...
void LedScript::setEffectName(const char *name)
{
    effectName = name;
}
...
void LedScript::reset(unsigned position)
{
    if (resumePosition)
...
void LedScript::getEffectName(std::string& name)
{
    name = effectName;
}


uint32_t LedScript::getMinTime()
{
    return (uint32_t)(minTime*1000);
}


bool LedScript::isInterruptible()
{
    return interruptible;
}


bool LedScript::isLocked()
{
    return locked;
}


unsigned LedScript::getCurrentPosition()
{
    return currentLine;
}
...
{
    // Array of commands that we consider preloadable
    static const char* cmds[] = {"$color", "$mintime", "$notinterruptible",
                                 "$resume", NULL};

    // A comment can be skipped altogether
    if (line[0] == '#')
        return true;

    // Any non-command automatically disqualifies
    if (line[0] != '$')
        return false;

    for (int i = 0; cmds[i] != NULL; ++i)
    {
        if (line.find(cmds[i]) == 0)
            return true;
    }
...
double LedScript::parseLine(const std::string& line)
{
    ...
    if (ss.fail())
    {
        LOG_DEBUG("Empty or illformed line (%d)", currentLine);
        return 0.0;
    }
