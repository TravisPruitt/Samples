...
const char* FileSystem::getAppPath()
{
#ifndef WIN32
    return "/mayhem";
#else
...
const char* FileSystem::getMediaPath()
{
#ifndef WIN32
    return "/var/mayhem/media";
#else
    return getAppDataPath();
#endif
}
...
/**
 *  Create and open temp file based on supplied template. Template will be modified
 *  to the name of the opened file.
 */
bool FileSystem::createTempFile(char* tmpl, std::ofstream& f)
{
#ifndef WIN32
    int fd = mkstemp(tmpl);
    if(fd == -1)
        return false;
    f.open(tmpl, std::ofstream::binary);
    close(fd);
#endif
 ...
 *
 *  @return  Size of the file or zero if aan error occures.
 */
