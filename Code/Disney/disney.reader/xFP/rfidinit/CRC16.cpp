...
uint16_t CRC16::ComputeCRC16_CCITT(const uint8_t* bytes, unsigned int length, bool invert)
{
    return ComputeCRC16_CCITT(bytes, length, 0, invert);
}

uint16_t CRC16::ComputeCRC16_CCITT(const uint8_t* bytes, unsigned int length, int offset, bool invert)
{
    InitializeTable();
    uint16_t crc = 0x6363;
