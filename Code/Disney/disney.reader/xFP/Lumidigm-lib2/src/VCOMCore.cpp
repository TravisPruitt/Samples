...
{
	Macro_Update_PIC_Firmware* pCommand = reinterpret_cast<Macro_Update_PIC_Firmware*>(V100CommandHandler::GetCommandHandler(pDev)->CreateCommand(_V100_COMMAND_SET(CMD_UPDATE_PIC_FIRMWARE)));
	// Set the command
	if(false == pCommand->SetData(pFirmwareStream, nFWStreamSize)) 
	{
		delete pCommand; return (V100_ERROR_CODE)(1);
	...
	pCommand = dynamic_cast<Macro_Update_PIC_Firmware*>(pResponse);
	return GEN_OK;
}

V100_ERROR_CODE V100_Script_Write(const V100_DEVICE_TRANSPORT_INFO *pDev, char* pFileToWrite, uint nFileSize)
{
	Atomic_Script_Write* pCommand = reinterpret_cast<Atomic_Script_Write*>(V100CommandHandler::GetCommandHandler(pDev)->CreateCommand((_V100_COMMAND_SET)CMD_SCRIPT_WRITE));
	_V100_FILE FileAttr;
#ifndef _GCC
	time_t lTime = 0;
	time( &lTime );
	uint(FileAttr.TimeStamp) = (uint)lTime;
#endif
	FileAttr.FileSize = nFileSize;
	FileAttr.FileType = FS_FILE_TYPE_FILE;
	FileAttr.Attribute = FS_FILE_TYPE_FILE;

	char pFileName[] = "script_set.xml";
	pCommand->SetFileAttr(FileAttr);
	pCommand->SetFileName(pFileName, (uint)strlen(pFileName));
	if (!pCommand->SetFileStream(pFileToWrite,FileAttr.FileSize))
		return GEN_ERROR_PARAMETER;
	...
	pCommand = dynamic_cast<Atomic_Script_Write*>(pResponse);
	delete pCommand;
	return GEN_OK;
}

V100_ERROR_CODE V100_Script_Read(const V100_DEVICE_TRANSPORT_INFO *pDev, char* pFileToRead, uint* pFileSize)
{
	Atomic_Script_Read* pCommand = reinterpret_cast<Atomic_Script_Read*>(V100CommandHandler::GetCommandHandler(pDev)->CreateCommand((_V100_COMMAND_SET)CMD_SCRIPT_READ));
	// Set File Name
	_V100_FILE FileAttr;
	char pFileName[] = "script_set.xml";
	pCommand->SetFileName(pFileName, (uint)strlen(pFileName));
	memset(&FileAttr, 0 , sizeof(_V100_FILE));
	ICmd* pResponse = (V100CommandHandler::GetCommandHandler(pDev)->IssueCommand((V100_DEVICE_TRANSPORT_INFO*)(pDev),pCommand));
	if(pResponse == NULL) return GEN_ERROR_COMM_TIMEOUT;
	Atomic_Error* pErr = dynamic_cast<Atomic_Error*>(pResponse);
	if(pErr)  // Handle Error
	...
	pCommand = dynamic_cast<Atomic_Script_Read*>(pResponse);
	memcpy(pFileToRead, pCommand->GetFileStream(), *(pCommand->GetFileStreamSize()));
	memcpy(&FileAttr, pCommand->GetFileAttr(), sizeof(_V100_FILE));
	*pFileSize = FileAttr.FileSize;
	delete pCommand;
	return GEN_OK;
}

V100_ERROR_CODE V100_Script_Play(const V100_DEVICE_TRANSPORT_INFO *pDev, char* pScriptName, LED_BRIGHTNESS brightness)
{
	Atomic_Script_Play* pCommand = reinterpret_cast<Atomic_Script_Play*>(V100CommandHandler::GetCommandHandler(pDev)->CreateCommand((_V100_COMMAND_SET)CMD_SCRIPT_PLAY));
	pCommand->SetScriptName(pScriptName);
	pCommand->SetBrightness(brightness);
	ICmd* pResponse = (V100CommandHandler::GetCommandHandler(pDev)->IssueCommand((V100_DEVICE_TRANSPORT_INFO*)(pDev),pCommand));
	if(pResponse == NULL) return GEN_ERROR_COMM_TIMEOUT;
	Atomic_Error* pErr = dynamic_cast<Atomic_Error*>(pResponse);
	if(pErr)  // Handle Error
	...
	pCommand = dynamic_cast<Atomic_Script_Play*>(pResponse);
	delete pCommand;
	return GEN_OK;
}
