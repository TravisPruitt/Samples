...
uint TransportUSB::Initialize(V100_DEVICE_TRANSPORT_INFO* pTransport)
{
    ...
    pTransport->hWrite = pTransport->hRead = (void*)open(deviceName, O_RDWR|O_NDELAY,0);
    handleToClose = (int)(pTransport->hWrite);
...
bool TransportUSB::ReadBytes(HANDLE hRead, void* pPacket, uint nPacketSize, uint mSecTimeout)
{
	...
	while(!bTimeOut)
	{
		...
		if(nRxSize == 0)
		{
			...
			if(nTOcount++ >= 5000)//8 sec timeout
			{
				bTimeOut = true;
