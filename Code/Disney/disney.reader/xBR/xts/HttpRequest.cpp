...
void HttpRequest::setPayloadFromFile(const char* filePath)
{
    ...
    {
        logs::curl("unable to open '%s' for HTTP payload\n", filePath);
    }
}
...
bool HttpRequest::send()
{
    ...
    _connection->easy_setopt(CURLOPT_HTTPHEADER, (void*)NULL);
    ...
    else if (_method == HTTP_POST)
    {
        ...
        _connection->easy_setopt(CURLOPT_POSTFIELDS, (void*)NULL);
    ...
    if (result == 0)
        logs::curl("response code = %d\n", getResponseCode());
    else
        logs::curl("HTTP request curl response %d (%s)\n", result, _errorDescription);
    return result == 0;
}
...
size_t HttpRequest::appendReceivedData(void* data, size_t length)
{
    ...
    logs::curl("resizing from %d to %d\n", beforeSize, beforeSize + length);
