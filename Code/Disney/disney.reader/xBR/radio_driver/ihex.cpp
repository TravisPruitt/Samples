...
ihexreader::ihexreader(char *filename) : ihex_file(filename), verbose(0)
{
};
...
pagemap_t* ihexreader::read(char *filename)
{
	if(filename)
		ihex_file = filename;
	return parsefile();
...
//	printf("Len %d, address %x, record type %d offs %x\n", len, addr, record_type, offs);
	if(record_type == 0) {
...
//			printf("1st len %d, 2nd len %d, len %d\n", first_len, second_len, len);
...
void ihexreader::parseline(std::string &s, pagemap_t &pagemap)
{
	...
	ss >> scode;
	if(scode != ':') {
		std::cout << "Unknown start code: '" << scode << "'\n";
		return;
	}
	while(ss.peek() != EOF) {
		char high, low;
...
pagemap_t* ihexreader::parsefile()
{
	std::ifstream f;
	std::string l;
	pagemap_t *pagemap;

	f.open(ihex_file, std::ifstream::in);
	if(!f.good())
		return NULL;
